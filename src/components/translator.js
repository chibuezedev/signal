    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        const response = await fetch("http://localhost:5000/translate", {
          method: "POST",
          headers: {
            // Send the token with the request for translation
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-md">
        <h1 className="text-3xl font-bold text-center mb-6">Signal</h1>
        <div className="space-y-4">
          ...
          {translatedLetter && (
            <div className="text-center">
              <h3 className="text-lg font-semibold">Translated Letter:</h3>
              <p className="text-4xl font-bold">{translatedLetter}</p>
            </div>
          )}
          {cameraPermission === "denied" && (
            <div
              ...
              role="alert"
            >
              <strong className="font-bold">Camera Permission Denied!</strong>
